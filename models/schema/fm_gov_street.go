...
// Street schema 对应 fm_gov_street 表
type GovStreet struct {
	ent.Schema
}

// 自定义表名
func (GovStreet) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "fm_gov_street"},
		entsql.WithComments(true), // 全局关闭字段注释同步
	}
}

func (GovStreet) Fields() []ent.Field {
	return []ent.Field{
		field.Int64("id").
			Unique().
			Immutable().
		...
		field.String("area_code").
			MaxLen(20).
			Default("").Comment("区县编号").
			StructTag(`json:"area_code"  db:"area_code"`),
...
func (GovStreet) Indexes() []ent.Index {
	return []ent.Index{
		// id 唯一
